...
// ═══════════════════════════════════════════════════════════
//  ESSENCE — CONSTANTES & HELPERS
// ═══════════════════════════════════════════════════════════
const FUEL_TYPES = [
  { key:"gazole_prix", label:"Gazole", icon:"🚛", color:"#fb923c" },
  { key:"sp95_prix",   label:"SP95",   icon:"⛽", color:"#ef4444" },
  { key:"e10_prix",    label:"E10",    icon:"🌿", color:"#4ade80" },
  { key:"sp98_prix",   label:"SP98",   icon:"🔵", color:"#60a5fa" },
  { key:"e85_prix",    label:"E85",    icon:"🌾", color:"#a3e635" },
  { key:"gplc_prix",   label:"GPL",    icon:"💎", color:"#e879f9" },
];

const STATION_BRANDS = {
  "INTERMARCHE":   { bg:"#E30613", fg:"#fff", short:"IN" },
  "LECLERC":       { bg:"#003f8f", fg:"#fff", short:"E." },
  "CARREFOUR":     { bg:"#0070f0", fg:"#fff", short:"CF" },
  "TOTAL ACCESS":  { bg:"#E8000D", fg:"#fff", short:"TA" },
  "TOTALENERGIES": { bg:"#E8000D", fg:"#fff", short:"TE" },
  "TOTAL":         { bg:"#E8000D", fg:"#fff", short:"TO" },
  "ESSO EXPRESS":  { bg:"#003399", fg:"#fff", short:"ES" },
  "ESSO":          { bg:"#003399", fg:"#fff", short:"ES" },
  "SHELL":         { bg:"#FBCE07", fg:"#000", short:"SH" },
  "BP":            { bg:"#006900", fg:"#fff", short:"BP" },
  "CASINO":        { bg:"#e31e24", fg:"#fff", short:"CA" },
  "SUPER U":       { bg:"#005baf", fg:"#fff", short:"SU" },
  "HYPER U":       { bg:"#005baf", fg:"#fff", short:"HU" },
  "U EXPRESS":     { bg:"#005baf", fg:"#fff", short:"UE" },
  "SYSTÈME U":     { bg:"#005baf", fg:"#fff", short:"SU" },
  "LIDL":          { bg:"#0050aa", fg:"#FBCE07", short:"LI" },
  "CORA":          { bg:"#e8751a", fg:"#fff", short:"CO" },
  "AUCHAN":        { bg:"#e63226", fg:"#fff", short:"AU" },
  "DYNEFF":        { bg:"#ff6600", fg:"#fff", short:"DY" },
  "NETTO":         { bg:"#e63226", fg:"#fff", short:"NE" },
  "RELAIS":        { bg:"#f59e0b", fg:"#000", short:"RE" },
  "OIL!":          { bg:"#ff6600", fg:"#fff", short:"OI" },
  "VITO":          { bg:"#7c3aed", fg:"#fff", short:"VI" },
  "ELAN":          { bg:"#0ea5e9", fg:"#fff", short:"EL" },
};

const getBrandInfo = (ensigne) => {
  if (!ensigne) return { bg:"#374151", fg:"#fff", short:"⛽" };
  const up = ensigne.toUpperCase();
  for (const [k, v] of Object.entries(STATION_BRANDS)) {
    if (up.includes(k)) return v;
  }
  return { bg:"#374151", fg:"#fff", short: ensigne.slice(0,2).toUpperCase() };
};
...
const normCoord = (v) => {
  ...
  return Math.abs(n) > 180 ? n / 100000 : n;
};
...
/* ── ESSENCE ── */
.fuel-best-scroll{display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none;}
.fuel-best-scroll::-webkit-scrollbar{display:none;}
.fuel-best-card{min-width:162px;background:var(--glass);border-radius:16px;padding:14px 16px;cursor:pointer;position:relative;flex-shrink:0;transition:all .2s;border:1px solid rgba(255,255,255,0.07);}
.fuel-best-card:hover{background:var(--glass2);transform:translateY(-2px);box-shadow:var(--shadow-glow);}
.fuel-dist-badge{position:absolute;top:10px;right:10px;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.12);border-radius:20px;padding:2px 8px;font-size:10px;font-weight:700;color:var(--text3);display:flex;align-items:center;gap:3px;}
.station-row{background:var(--glass);border:1px solid var(--border);border-radius:14px;padding:14px 16px;margin-bottom:8px;display:flex;align-items:center;gap:14px;transition:all .18s;}
.station-row:hover{background:var(--glass2);border-color:var(--border2);}
.station-prices{display:flex;gap:5px;overflow-x:auto;scrollbar-width:none;flex-shrink:0;}
.station-prices::-webkit-scrollbar{display:none;}
.price-cell{min-width:68px;text-align:center;flex-shrink:0;}
.price-inner{background:rgba(255,255,255,0.04);border:1px solid transparent;border-radius:9px;padding:5px 4px;}
.price-inner.best{border-color:transparent;}
.fuel-header-row{display:flex;align-items:center;gap:14px;padding:6px 16px;margin-bottom:2px;}
.nav-opt-btn{display:flex;align-items:center;gap:14px;padding:14px 16px;margin-bottom:8px;background:var(--glass);border:1px solid var(--border);border-radius:13px;text-decoration:none;color:var(--text);transition:all .18s;cursor:pointer;width:100%;}
.nav-opt-btn:hover{background:var(--glass2);border-color:var(--border2);}
#leaflet-map-container .leaflet-popup-content-wrapper{background:#1a1635;border:1px solid rgba(255,255,255,0.15);border-radius:12px;color:#ede9f8;font-family:'Outfit',sans-serif;}
#leaflet-map-container .leaflet-popup-tip{background:#1a1635;}
#leaflet-map-container .leaflet-popup-close-button{color:#ede9f8;}
@media(max-width:880px){
  .fuel-header-row{display:none;}
  .station-row{flex-wrap:wrap;gap:10px;}
  .station-prices{width:100%;}
}
...
@media(max-width:880px){
  ...
  .bnav-item{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;padding:4px 6px;border-radius:10px;cursor:pointer;transition:all .18s;font-size:9px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.3px;min-width:44px;position:relative;}
...
export default function App() {
  ...
  const pageTitles = { dashboard: "Tableau de bord", incomes: "Revenus", expenses: "Dépenses", bills: "Factures", stats: "Statistiques", essence: "Prix Carburants ⛽", settings: "Réglages" };
...
function BrandIcon({ ensigne, size = 44 }) {
  const b = getBrandInfo(ensigne);
  return (
    <div style={{
      width:size, height:size, borderRadius:Math.round(size*.28),
      background:b.bg, color:b.fg,
      display:"flex", alignItems:"center", justifyContent:"center",
      fontSize:Math.round(size*.38), fontWeight:900,
      flexShrink:0, letterSpacing:-0.5,
      boxShadow:`0 2px 10px ${b.bg}55`,
    }}>{b.short}</div>
  );
}

function NavOptionsModal({ station, onClose }) {
  const lat = normCoord(station.latitude);
  const lng = normCoord(station.longitude);
  const name = station.ensigne || station.adresse || "Station";
  const opts = [
    { label:"Apple Plans",  sub:"Recommandé sur iPhone / Mac",   icon:"🍎", href:`https://maps.apple.com/?daddr=${lat},${lng}&dirflg=d` },
    { label:"Google Maps",  sub:"Disponible sur tous appareils", icon:"🗺️", href:`https://maps.google.com/maps?daddr=${lat},${lng}` },
    { label:"Waze",         sub:"Navigation trafic temps réel",  icon:"🔵", href:`https://waze.com/ul?ll=${lat},${lng}&navigate=yes` },
  ];
  return (
    <ModalWrap close={onClose} title="🗺️ Lancer l'itinéraire">
      <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:12, padding:"12px 16px", marginBottom:16 }}>
        <div style={{ fontWeight:700, fontSize:14 }}>{name}</div>
        <div style={{ fontSize:12, color:"var(--text3)", marginTop:2 }}>{station.adresse}{station.ville?`, ${station.ville}`:""}</div>
      </div>
      {opts.map(o => (
        <a key={o.label} href={o.href} target="_blank" rel="noopener noreferrer" className="nav-opt-btn">
      ...
      <button className="btn btn-ghost" style={{ width:"100%", marginTop:4 }} onClick={onClose}>Fermer</button>
    </ModalWrap>
  );
}

function FuelMap({ stations, userLat, userLng }) {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      const L = window.L;
      const center = (userLat && userLng) ? [userLat, userLng] : [46.6, 1.9];
      const zoom   = (userLat && userLng) ? 12 : 6;
      ...
      if (userLat && userLng) {
        const ui = L.divIcon({
          html:`<div style="width:20px;height:20px;background:#a78bfa;border:3px solid white;border-radius:50%;box-shadow:0 0 12px rgba(167,139,250,0.8)"></div>`,
      ...
      stations.forEach(st => {
        const lat = normCoord(st.latitude);
        const lng = normCoord(st.longitude);
        if (!lat || !lng) return;
        const b = getBrandInfo(st.ensigne);
        const dist = (userLat && userLng) ? fmtDist(haversineKm(userLat, userLng, lat, lng)) : null;
        const prices = FUEL_TYPES.filter(f => st[f.key] && parseFloat(st[f.key]) > 0)
          .map(f => `<span style="display:inline-block;margin:2px 3px 2px 0;background:#f1f5f9;border-radius:5px;padding:2px 6px;font-size:11px"><b>${f.label}</b> ${parseFloat(st[f.key]).toFixed(3)}€</span>`).join("");

        const icon = L.divIcon({
          html:`<div style="width:38px;height:38px;background:${b.bg};color:${b.fg};border:2.5px solid white;border-radius:11px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:900;font-family:Outfit,sans-serif;box-shadow:0 3px 10px rgba(0,0,0,0.45);cursor:pointer">${b.short}</div>`,
          className:"", iconSize:[38,38], iconAnchor:[19,19],
        });

        L.marker([lat, lng], { icon }).addTo(map).bindPopup(
          L.popup({ maxWidth:260 }).setContent(`
            <div style="font-family:Outfit,sans-serif;padding:6px 2px">
              <div style="font-weight:800;font-size:15px;margin-bottom:3px">${st.ensigne || "Station"}</div>
              <div style="font-size:11px;color:#888;margin-bottom:8px">${st.adresse||""}${st.ville?", "+st.ville:""}${dist?" · 📍 "+dist:""}</div>
              <div style="margin-bottom:10px">${prices||"<span style='color:#aaa;font-size:12px'>Prix non disponibles</span>"}</div>
              <a href="https://maps.google.com/maps?daddr=${lat},${lng}" target="_blank"
                style="display:block;text-align:center;background:linear-gradient(135deg,#a78bfa,#f472b6);color:white;padding:8px;border-radius:9px;text-decoration:none;font-weight:700;font-size:13px">
  ...
  return (
    <div id="leaflet-map-container" ref={mapRef}
      style={{ width:"100%", height:500, borderRadius:16, overflow:"hidden", border:"1px solid var(--border)" }} />
  );
}

function Essence() {
  const [query, setQuery]           = useState("52100");
  ...
  const doFetch = async (url) => {
    ...
    try {
      ...
      setTotalFound(json.total_count ?? (json.results?.length ?? 0));
    } catch(e) {
      setError("Impossible de récupérer les données. Vérifiez votre connexion internet.");
    }
    setLoading(false);
  };

  const fetchByPostal = (cp) => {
    const w = encodeURIComponent(`cp like "${cp.trim().slice(0,5)}%"`);
    doFetch(`https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets/prix-des-carburants-en-france-flux-instantane-v2/records?where=${w}&limit=50&timezone=Europe%2FParis`);
  };

  const fetchByGeo = (lat, lng) => {
    const w = encodeURIComponent(`distance(geom, geom'POINT(${lng} ${lat})', 15000m)`);
    doFetch(`https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets/prix-des-carburants-en-france-flux-instantane-v2/records?where=${w}&limit=50&timezone=Europe%2FParis`);
  };
  ...
  const locateUser = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        ...
        setLocating(false);
        fetchByGeo(coords.latitude, coords.longitude);
      },
      () => { setLocating(false); setError("Localisation refusée ou indisponible. Vérifiez les permissions."); },
      { enableHighAccuracy:true, timeout:8000 }
    );
  };

  useEffect(() => { fetchByPostal(query); }, []);
  ...
  const stationsWithDist = useMemo(() => {
    ...
    return stations.map(st => {
      const lat = normCoord(st.latitude);
      const lng = normCoord(st.longitude);
      if (lat == null || lng == null) return st;
      return { ...st, _dist: haversineKm(userLat, userLng, lat, lng) };
    }).sort((a, b) => (a._dist ?? 9999) - (b._dist ?? 9999));
  }, [stations, userLat, userLng]);

  const bestPrices = useMemo(() => FUEL_TYPES.map(fuel => {
    const candidates = stationsWithDist.filter(st => st[fuel.key] != null && parseFloat(st[fuel.key]) > 0);
    if (!candidates.length) return { ...fuel, station:null, price:null };
    const best = candidates.reduce((a, b) => parseFloat(a[fuel.key]) < parseFloat(b[fuel.key]) ? a : b);
    return { ...fuel, station:best, price:parseFloat(best[fuel.key]) };
  }), [stationsWithDist]);

  const isBestStation = (st, fuelKey) => {
    const b = bestPrices.find(x => x.key === fuelKey);
    if (!b?.station) return false;
    return b.station === st || (b.station.adresse === st.adresse && b.station.ville === st.ville);
  };
  ...
  return (
    <div className="fade-up">
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <div style={{ flex:1, display:"flex", gap:8, minWidth:180 }}>
            <input value={query} onChange={e => setQuery(e.target.value)}
              onKeyDown={e => e.key==="Enter" && fetchByPostal(query)}
              placeholder="Code postal (ex: 52100)" style={{ flex:1 }} />
            <button className="btn btn-primary btn-sm" onClick={() => fetchByPostal(query)} disabled={loading}>
              {loading ? <span className="spin" style={{display:"inline-block"}}>⟳</span> : "🔍 Chercher"}
            </button>
          </div>
          <button className="btn btn-ghost btn-sm" onClick={locateUser} disabled={locating}
          ...
          <div className="filter-bar">
            {[["list","📋 Liste"],["map","🗺️ Carte"]].map(([v,l]) => (
              <div key={v} className={`filter-chip ${viewMode===v?"active":""}`} onClick={() => setViewMode(v)}>{l}</div>
            ))}
          </div>
        </div>
        {error && <div style={{ marginTop:10, color:"var(--red)", fontSize:13 }}>⚠️ {error}</div>}
        {totalFound !== null && !loading && (
          <div style={{ marginTop:8, fontSize:12, color:"var(--text3)" }}>
            {totalFound} station{totalFound!==1?"s":""} trouvée{totalFound!==1?"s":""}
            {userLat ? " dans un rayon de 15 km · 📍 Position GPS active" : ` pour le code postal "${query}"`}
          </div>
        )}
      </div>

      {/* ── Meilleurs prix (style photo 3) ── */}
      {!loading && stationsWithDist.length > 0 && (
        <div style={{ marginBottom:22 }}>
          <div style={{ fontSize:11, color:"var(--text3)", textTransform:"uppercase", letterSpacing:1, marginBottom:10, fontWeight:700 }}>
            ⭐ Meilleurs prix — cliquer pour l'itinéraire
          </div>
          <div className="fuel-best-scroll">
            {bestPrices.filter(b => b.price).map(b => {
              const dist = b.station?._dist;
              return (
                <div key={b.key} className="fuel-best-card" style={{ borderColor:`${b.color}25` }}
                  onClick={() => b.station && setNavStation(b.station)}>
                  {dist != null && <div className="fuel-dist-badge">📍 {fmtDist(dist)}</div>}
                  <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8, paddingRight: dist!=null ? 55:0 }}>
                    <span style={{ fontSize:17 }}>{b.icon}</span>
                    <span style={{ fontSize:11, fontWeight:700, color:b.color, textTransform:"uppercase", letterSpacing:.5 }}>{b.label}</span>
                  </div>
                  <div className="stat-num" style={{ fontSize:28, color:b.color, lineHeight:1, marginBottom:8 }}>
                    {b.price.toFixed(3)}<span style={{ fontSize:13, opacity:.65 }}>€/L</span>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <BrandIcon ensigne={b.station?.ensigne} size={24} />
                    <div style={{ minWidth:0 }}>
                      <div style={{ fontSize:11, fontWeight:700, color:"var(--text)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                        {b.station?.ensigne || "Station"}
                      </div>
        ...
        <div style={{ marginBottom:16 }}>
          {loading ? (
            <div className="card" style={{ textAlign:"center", padding:60 }}>
              <div className="spin" style={{ fontSize:34, display:"inline-block", marginBottom:14 }}>⟳</div>
              <div style={{ color:"var(--text3)" }}>Chargement de la carte…</div>
            </div>
          ) : stationsWithDist.length > 0 ? (
            <FuelMap stations={stationsWithDist} userLat={userLat} userLng={userLng} />
          ) : (
            <div className="card empty-state"><div className="empty-icon">🗺️</div>Aucune station à afficher sur la carte</div>
          )}
        </div>
      )}
        ...
        <>
          {loading && (
            <div className="card" style={{ textAlign:"center", padding:44 }}>
              <div className="spin" style={{ fontSize:34, display:"inline-block", marginBottom:14 }}>⟳</div>
              <div style={{ color:"var(--text3)" }}>Récupération des prix carburants…</div>
            </div>
          )}
          {!loading && stationsWithDist.length === 0 && !error && (
            <div className="card empty-state"><div className="empty-icon">⛽</div>Aucune station trouvée pour cette recherche</div>
          )}
          {!loading && stationsWithDist.length > 0 && (
            <>
              {/* En-tête colonnes */}
              <div className="fuel-header-row">
                <div style={{ width:44 }} /><div style={{ width:44 }} />
                <div style={{ flex:1 }} />
                <div style={{ width:80 }} />
                {FUEL_TYPES.map(f => (
                  <div key={f.key} style={{ minWidth:68, textAlign:"center", fontSize:10, fontWeight:700, color:f.color, textTransform:"uppercase", letterSpacing:.5 }}>
                    {f.icon} {f.label}
                  </div>
                ))}
                <div style={{ width:42 }} />
              ...
              {stationsWithDist.map((st, idx) => (
                <div key={st.id || idx} className={`station-row fade-up stagger-${(idx%5)+1}`}>
                  <BrandIcon ensigne={st.ensigne} size={44} />
                  ...
                  {st._dist != null && (
                    <div style={{
                      ...
                      color:"var(--purple)", flexShrink:0, whiteSpace:"nowrap",
                    }}>📍 {fmtDist(st._dist)}</div>
                  )}
                  <div className="station-prices">
                    {FUEL_TYPES.map(f => {
                      const price = st[f.key];
                      const best  = isBestStation(st, f.key);
                      return (
                        <div key={f.key} className="price-cell">
                          {price ? (
                            <div className="price-inner" style={{ background: best ? `${f.color}14` : "rgba(255,255,255,0.04)", borderColor: best ? `${f.color}45` : "transparent" }}>
                              <div className="stat-num" style={{ fontSize:14, color: best ? f.color : "var(--text)", fontWeight: best ? 800 : 600 }}>
                                {parseFloat(price).toFixed(3)}
                              </div>
                              {best && <div style={{ fontSize:9, color:f.color, fontWeight:700, marginTop:1 }}>✓ Moins cher</div>}
                            </div>
                          ) : (
                            <div style={{ fontSize:14, color:"var(--text3)", padding:"5px 0", textAlign:"center" }}>—</div>
                          )}
                        </div>
                      );
                    })}
                  </div>
                  <button className="btn-icon" onClick={() => setNavStation(st)} title="Lancer l'itinéraire"
                    style={{ color:"var(--purple)", borderColor:"rgba(167,139,250,0.25)", background:"rgba(167,139,250,0.08)", flexShrink:0 }}>
                    🗺️
                  </button>
                </div>
              ))}
      ...
      {navStation && <NavOptionsModal station={navStation} onClose={() => setNavStation(null)} />}
    </div>
  );
}
